-- ============================================================================
-- ERA — Esquema inicial (DDL para ejecución manual en MySQL Workbench)
-- Motor: MySQL 8.0 · InnoDB · utf8mb4
--
-- Script autocontenido para el SQL Editor de Workbench: además del DDL de la
-- migración Flyway `resources/db/migration/V1__init_schema.sql`, incluye el
-- `CREATE DATABASE` + `USE era_db` iniciales (única diferencia con la migración).
--
-- PASOS:
--   1. Abre el script (File > Open SQL Script) con una conexión al servidor y
--      ejecuta TODO (Ctrl+Shift+Enter). Es seguro re-ejecutarlo: `CREATE DATABASE`
--      y `USE` son idempotentes; los `CREATE TABLE` solo fallarán si la tabla ya
--      existe (esperado).
--   2. Si prefieres otra base, cambia el nombre tras `USE` y usa el mismo valor
--      en `DB_NAME` de `.env.example`.
--   3. NO incluye datos de prueba (INSERT); solo la estructura.
--
-- Convenciones:
...
CREATE DATABASE IF NOT EXISTS era_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE era_db;
